...
- **npm クールダウンで create-astro / `astro add` / `npx` ツールが失敗** → クールダウン型を見極めて env 上書き(変換型=`min_release_age=0`/直書き型=`before=未来日`)、必要ならピン下げ+手動配線(セットアップ)。

## セットアップ(1プロジェクト1回・**バックグラウンド先行**)

**着手前に前提を2つプローブ**(どちらも BG レーンを沈黙させないため)。① `npm config get min-release-age before`——**`before` が非 null ならクールダウン環境**。npm11 は `min-release-age=N` を **`before=今−N日` に変換**するため、`min-release-age` 側は**効いていても null と表示される**(罠:`get` の値だけでは型を判別できない)。**確実な判別は .npmrc を grep**(`grep -nE 'min-release-age|before' "$(npm config get userconfig)" ./.npmrc`)——`min-release-age=N` 行があれば**変換型**、`before=<固定日付>` 直書きなら**直書き型**(③で上書き方法が逆になる)。型に応じた回避を**最初から織り込んで**から動く(暗号的な install 失敗を待たない)。② `printenv UNSPLASH_ACCESS_KEY`——**空なら画像レーン(B)は黙って全 fetch 失敗する**ので、**今すぐユーザーに伝えて export を促す**(数秒で直る。フェーズ4で全スロットがプレースホルダ箱だと気づくのは画像レーン丸ごと手戻り)。設計・scaffold はキー不要なので、伝えつつ並行で進めてよい。
...
- **npm クールダウン環境**(`min-release-age`/`before`)は3箇所を壊す:①create-astro が ETARGET → `package.json` の `"astro"` を **`^X.Y.0` に下げて** install し直す。②**`astro add <x>` も依存解決に失敗**(config 未配線・CSS 未生成のまま落ちる)→ 統合を手動 install して `astro.config` に挿す(例:`@tailwindcss/vite` を入れ `vite:{plugins:[tailwindcss()]}`)+ `global.css` 自作。③**`npx <tool>`(unsplash-fetch 等)が "No versions available" でブロック**(キーがあっても)→ その呼び出しだけ env で上書き(グローバル設定は不変):**変換型は `npm_config_min_release_age=0 npx …`**、**直書き型は `npm_config_before=2099-01-01 npx …`**(空文字 `npm_config_before=` は無視され不可)。両者は**排他フラグで同時指定不可**(`--before cannot be provided when using --min-release-age`)——②の grep で確定した実キーに合わせて1つだけ選ぶ。①②の create/install も**同じ env を前置すれば回避でき**、効かない時だけピン下げ+手動配線にフォールバック。
- 要件:Node + `UNSPLASH_ACCESS_KEY`(画像は **`npx unsplash-fetch`**・導入不要)。**鍵は着手前にプローブ**(上記)。さらにレーンB は**最初の1スロットを撮ってファイルが実際に落ちたか確認**してから残り全スロットへ展開すると、鍵切れ/クールダウン/ネット断を**実画像1枚で炙り出せる**(env プローブを通り抜けた沈黙も捕まる)。
...
**並列の不変条件(衝突防止)**:ファンアウトは**キット凍結後にのみ起動**する。各エージェントは**自分の `src/pages/<name>.astro` だけを書く**。共有ファイル(`global.css`・`Layout`・共通コンポーネント)は**読み取り専用**——不足や修正が要るキットは並列を止め、直列スパインに戻して直し、**`## キット契約` を再発行してから**再ファンアウトする(複数エージェントの同時追記は競合する)。

**ファンアウト直後ゲート(型=契約・fail-fast)**:全ページ合成後、視覚ループに入る前に **`npx astro check` を緑にする**。`astro build` は transpile-only で**型を検査しない**(`heading` に数値を渡しても "123" のまま通る——実測:build 成功 / check が `ts(2322)` で検出)。契約違反(prop名・型・必須欠落)はビルドもスクショも通り抜け、放置すると最も高価なフェーズ4を巡回浪費して P5 でようやく露見する。**最安のゲートを最もリスキーな瞬間=手渡し直後に置く**(型由来の崩れを批評に持ち込まない)。
...
4. 各修正を**局所/系統に分類**して当てる(並列可)→ 再ビルド → 再スクショ:
   - **局所**(そのページのデータ/合成由来)→ per-page でピンポイント修正。
   - **系統**(共有キット部品由来=複数ページに同症状・blast radius≧2)→ **並列を止め、直列スパインで該当部品を1回だけ直し、`## キット契約` を再発行 → 該当ページのみ再スクショ**。per-page で各自が共有部品をパッチするのは**禁止**(競合/様式分裂で停止条件『一貫性』を自ら破る)。
   - **不変条件(P3と同じ)**:並列批評中も共有ファイル(`global.css`・`Layout`・共通コンポーネント)は**読み取り専用**=触るのは自分のページだけ。
