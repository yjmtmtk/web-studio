...
cd <proj> && npx astro add tailwind --yes
...
5. **閾値(ルーブリック)を満たすか予算切れまでループ**。重要導線(ヒーロー/主要CTA)は厚く、定型は薄く。

> ルーブリック例:階層(サイズ/位置で作れているか)・垂直リズム/余白の均一さ・一貫性・可読性/コントラスト・
> レスポンシブ・モーションの上品さ・アクセシビリティ。**測れるから回せる。**
